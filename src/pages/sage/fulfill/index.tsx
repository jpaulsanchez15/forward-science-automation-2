...
const FulfillPage: NextPage = () => {
  ...
  return (
    <>
      <Head>
        <title>Forward Science Automation | Fulfill Sage</title>
      </Head>
      <main className="m-auto flex min-h-screen  flex-col items-center justify-center">
        <h1 className="mb-6 text-center text-4xl font-bold">Fulfill Sage</h1>
        <div>
          {data.length > 0 ? (
                ...
                <div className="max-w-2xlpy-32 mx-auto sm:py-48 lg:py-32">
                  <div className="hidden sm:mb-8 sm:flex sm:justify-center">
                    <div className="hover:bg-vanilla relative rounded-full bg-white px-3 py-1 text-sm leading-6 text-gray-600 ring-1 ring-gray-900/10 hover:bg-gray-400 hover:ring-gray-900/20 dark:bg-white dark:text-black dark:hover:bg-gray-400">
                      ...
                      <Link href="/sage/create" className="font-semibold">
...
const Orders = ({
  ...
  const deletedOrder = api.sageOrder.deleteOrder.useMutation({
    onSuccess: () => {
      toast({
  ...
  const fileAwayOrder = api.sageOrder.completeOrder.useMutation({
    onSuccess: () => {
      toast({
  ...
  const handleCreateOrder = async () => {
    ...
    try {
      const res = await fetch("/api/ordoro/sage/createSageOrder", {
        method: "POST",
        headers: {
  ...
  const handleCreateLabel = async () => {
    ...
    try {
      const label = await formatOrder(order.lines, false);
      ...
      const labelCreated = await fetch("/api/ordoro/sage/createSageLabel", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
